#include "ARWaveObject.h"


ARWaveObject::ARWaveObject(void)
{
	this->xGird = this->yGird = 2;
	this->init();
}

ARWaveObject::ARWaveObject(int x,int y)
{
...
void ARWaveObject::draw()
{
	ARMesh::draw();
	posY += 0.1f;
	this->getMaterial()->setUniform1f("time",posY);
}
